...
const EventError_1 = require("./exceptions/EventError");
class Battlefield extends events_1.EventEmitter {
    ...
    async connect(forceReconnect = true) {
    ...
    async reconnect(maxAttempts = -1, timeout = 1000) {
        ...
        while ((attempts++ < maxAttempts || maxAttempts <= 0) && !this.abortReconnectAction) {
            ...
            try {
                ...
                this.isReconnecting = false;
                this.abortReconnectAction = false;
                this.emit("reconnect", { attempt: attempts, success: true });
                return;
            }
            catch (e) {
                this.emit("reconnect", { attempt: attempts, success: false });
            }
        }
        this.isReconnecting = false;
        this.abortReconnectAction = false;
    ...
    async onSquadChange(words) {
        ...
        if (!player)
            return; //player left drop silently....
    ...
    async onTeamChange(words) {
        ...
        if (player) {
            ...
            this.emit("error", new EventError_1.EventError(`could not find player ${name}`, "onTeamChange"));
        }
    }
    ...
    async playerOnSpawn(words) {
        ...
        if (player) {
            ...
            this.emit("error", new EventError_1.EventError(`could not find player ${name}`, "playerOnSpawn"));
        }
    }
